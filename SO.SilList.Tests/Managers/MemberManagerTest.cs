...
namespace SO.SilList.Tests.Managers
{
    ...
    public class MemberManagerTest
    {
        ...
        {
            string searchString = "random autogenerated test user" + Guid.NewGuid().ToString();
            ...
            //1. assume insert is tested and works find
            MemberVo testMember = new MemberVo();
            testMember.firstName = searchString;
            testMember.siteId = siteId;
            var inserted = memberManager.insert(testMember);
            if (inserted == null)
            {
                Assert.Fail("Can not insert in Members table to test the memberManager's search function");
                return;
            }

            //2. try to find new autogenerated record
            var memberVm = new MemberVm();
            memberVm.keyword = searchString;
            memberVm.paging.pageNumber = 1;
            memberVm.filterIsActive = true;

            var searchRes = memberManager.search(memberVm);

            Assert.IsTrue(searchRes != null);
            if (searchRes == null)
                return;

            //3. delete added user
            memberManager.delete(inserted.memberId);
        }
